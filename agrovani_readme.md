...
### 1. Clone & Install
```bash
git clone https://github.com/your-username/AgroVani.git
cd AgroVani
npm install
```
...
Update your `.env` with your Supabase credentials and adapter keys:
```env
NEXT_PUBLIC_SUPABASE_URL=your_supabase_project_url
NEXT_PUBLIC_SUPABASE_ANON_KEY=your_supabase_anon_key
SUPABASE_SERVICE_ROLE_KEY=your_supabase_service_role_key

# External Weather / Agriculture APIs
NEXT_PUBLIC_WEATHER_API_KEY=your_weather_api_key
```

### 3. Database Migration
Run the SQL script located at `supabase/schema.sql` inside your Supabase project's **SQL Editor** to create all tables, enum types, and row-level security (RLS) rules.
