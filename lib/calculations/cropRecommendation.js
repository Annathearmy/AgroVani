...
export const PRODUCT_CATALOG = [
  ...
  { name: 'Isabion', type: 'Biostimulant', composition: 'Amino acids + nutrient peptides', targets: 'Frost, heat, drought, and flowering/fruit-set support', category: 'biostimulant', dosage: '2 ml/L foliar spray; calculate total volume from the crop label' },
  ...
  { name: 'Quantis', type: 'Biostimulant', composition: 'Potassium, calcium, and organic carbon', targets: 'Photosynthesis support during heat stress', category: 'biostimulant', dosage: '2 ml/L foliar spray; calculate total volume from the crop label' },
]
  ...
  return PRODUCT_CATALOG
    ...
    .map(({ name, type, composition, targets, dosage }) => ({
      ...
      dosageGuidance: dosage
        ? `${dosage}. Confirm the current registered India label, crop, target, water volume, and safety interval before use.`
        : 'No registered dose is available for this candidate. Confirm the current India label before use.',
    }))
}
...
export function recommendProduct({ diurnal, night, frost, di, crop }) {
  ...
  if (diurnal > 4 || night > 4 || frost > 0) {
    ...
    return {
      ...
      options: productCandidates('biostimulant', crop).filter((option) => option.name === stressProduct),
      category: 'stress',
