...
export function computeFieldReadiness({ residueTons, machinery = [], activeListings = 0, activeOrders = 0 }) {
  const totalMachines = machinery.length;
  const availableMachines = machinery.filter((item) => item.available !== false).length;
  const machineScore = totalMachines ? (availableMachines / totalMachines) * 100 : 0;
  const stockScore = Math.min(100, activeListings * 15 + activeOrders * 25);
  const readiness = totalMachines ? Math.round(machineScore) : null;
  const demandScore = Math.min(100, Math.round(stockScore + Math.min(40, Number(residueTons || 0) * 5)));
  return {
    ...
    activeListings,
    activeOrders,
    dataSource: 'live machinery, marketplace listings, and marketplace orders',
  };
}
